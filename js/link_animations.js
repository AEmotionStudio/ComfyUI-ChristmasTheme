...
import { initSettingsCache, updateCache, loadSettingFromStorage, getSetting, COLOR_SCHEMES } from "./settings-cache.js";
  ...
  async setup() {
    ...
    const TimingManager = {
      ...
      update() {
        ...
        const rawDelta = Math.min((now - State.lastFrame) / 1e3, 1 / 30);
        State.lastFrame = now;
        this.frameCount++;
        this.smoothDelta = this.smoothDelta * AnimationState.smoothFactor + rawDelta * (1 - AnimationState.smoothFactor);
